...
describe('FlexibleTokenSale End-To-End Scenario', () => {
   ...
   const TOKEN_TOTALSUPPLY     = new BigNumber("500000000").times(DECIMALS_FACTOR)

   const CONTRIBUTION_MIN      = new BigNumber(0.1).times(DECIMALS_FACTOR)


   // Presale configuration
   const PRESALE_TOKENS                 = new BigNumber("15000000").times(DECIMALS_FACTOR)
   const PRESALE_TOKENSPERKETHER        = 1700000
   const PRESALE_BONUS                  = 2000
   const PRESALE_MAXTOKENSPERACCOUNT    = new BigNumber(17000).times(DECIMALS_FACTOR)
   const PRESALE_STARTTIME              = Moment().add(1, 'months')
   const PRESALE_ENDTIME                = Moment().add(2, 'months')

   // Public sale configuration
   const PUBLICSALE_TOKENS              = new BigNumber("85000000").times(DECIMALS_FACTOR)
   const PUBLICSALE_TOKENSPERKETHER     = 1700000
   const PUBLICSALE_BONUS               = 0
   const PUBLICSALE_MAXTOKENSPERACCOUNT = new BigNumber(50000).times(DECIMALS_FACTOR)
   ...
   context('After Presale', async() => {
      ...
      it('Reclaim unsold tokens', async () => {
         ...
         assert.equal(ownerTokensAfter.minus(ownerTokensBefore), saleTokensBefore)
      })
   })
   ...
   context('During Public Sale', async () => {
      ...
      it('Raise per account contribution limit', async () => {
         await sale.methods.setMaxTokensPerAccount(PUBLICSALE_MAXTOKENSPERACCOUNT.times(2)).send({ from: owner })
      })
      ...
      it('Contributor buys all remaining tokens', async () => {
         ...
         assert.equal(new BigNumber(await sale.methods.totalTokensSold().call()), tokensSoldPresale.plus(PUBLICSALE_TOKENS))
      })
   })
