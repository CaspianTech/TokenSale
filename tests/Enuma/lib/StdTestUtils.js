...
function tokensFromWei(tokensPerKEther, weiAmount, bonus) {
   return weiAmount.times(tokensPerKEther).times(bonus.plus(10000)).div(1000).div(10000)
}
...
function weiFromTokens(tokensPerKEther, tokenAmount, bonus) {
   return tokenAmount.times(1000).times(10000).div(tokensPerKEther.times(bonus.plus(10000)))
}
...
module.exports.checkTransferFromBatch = (receipt, batchSize) => {

   TestLib.checkStatus(receipt)

   assert.equal(Object.keys(receipt.events).length, batchSize + 1)
   assert.equal(typeof receipt.events.TransferFromBatchCompleted, 'object')
   const eventArgs = receipt.events.TransferFromBatchCompleted.returnValues
   assert.equal(Object.keys(eventArgs).length, 2)
   assert.equal(eventArgs._batchSize, batchSize)
}
...
module.exports.checkGrantTimeLockAllocation = (receipt, beneficiary, amount) => {
...
module.exports.checkProcessTimeLockAllocation = (receipt, beneficiary, amountTransferred) => {
   return module.exports.checkProcessAllocation(receipt, beneficiary, amountTransferred)
}
...
module.exports.checkProcessAllocation = (receipt, beneficiary, amountTransferred) => {
   ...
   assert.equal(Object.keys(eventArgs).length, 4)
   assert.equal(eventArgs._beneficiary, beneficiary)
   assert.equal(eventArgs._amountTransferred, amountTransferred)
}
...
module.exports.buyTokens = async (token, sale, owner, wallet, DECIMALS_FACTOR, from, to, amount) => {
   ...
   if (maxTokensPerAccount.gt(0)) {
      ...
      const quotaLeft = maxTokensPerAccount.minus(toTokensBefore)
   ...
   if (amount == -1) {
      ...
      tokensPerKEther = maxTokens.times(1000).div(DECIMALS_FACTOR).integerValue(BigNumber.ROUND_FLOOR)
      ...
      expectedTokenCost = weiFromTokens(tokensPerKEther, expectedTokens, bonus).integerValue(BigNumber.ROUND_FLOOR)
   } else {
      transferAmount    = amount
      expectedTokens    = tokensFromWei(tokensPerKEther, transferAmount, bonus).integerValue(BigNumber.ROUND_FLOOR)
