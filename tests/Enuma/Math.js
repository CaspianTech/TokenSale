...
describe('Math Library', () => {
   ...
   before(async () => {
      ...
      uint256Max = new BigNumber(2).pow(256).minus(1)
   })
   ...
   context('sub', async () => {
      ...
      it('sub(uint256, 1)', async () => {
         assert.equal(new BigNumber(await math.methods.sub(uint256Max, 1).call()), uint256Max.minus(1))
      })
...
//    - div(uint256, uint256)
   context('div', async () => {
      ...
      it('div(uint256, 2)', async () => {
         assert.equal(new BigNumber(await math.methods.div(uint256Max, 2).call()), uint256Max.div(2).integerValue(BigNumber.ROUND_FLOOR))
      })
