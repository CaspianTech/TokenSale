...
describe('FlexibleTokenSale Contract - buyTokens tests', () => {
   ...
   const TOKEN_TOTALSUPPLY = new BigNumber("10000000").times(DECIMALS_FACTOR)
   ...
   const redeploy = async () => {
      ...
      await token.methods.transfer(sale._address, new BigNumber(5000000).times(DECIMALS_FACTOR)).send({ from: owner })
   }
   ...
   context('buyTokens', async () => {
      ...
      it('buyTokens with minimum contribution - 1 wei', async () => {
         await TestLib.assertCallFails(buyTokens(account1, account1, contributionMin.minus(1)))
      })
      ...
      it('buyTokens with more ETH than maxTokensPerAccount allows', async () => {
         ...
         await sale.methods.setMaxTokensPerAccount(balance.plus(1)).send({ from: owner })
      ...
      it('buyTokens with less tokens left than maxTokensPerAccount', async () => {
         ...
         await sale.methods.setMaxTokensPerAccount(balance.plus(1000000)).send({ from: owner })
